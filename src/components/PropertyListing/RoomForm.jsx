...
const RoomForm = ({ propertyId }) => {
  ...
  const { addRoomToProperty } = useContext(ListingContext);
  ...
  const removeAmenity = (index) => {
    ...
    // Shift amenities to fill the gap
  ...
  const validateForm = () => {
    ...
    if (!room.description.trim()) newErrors.description = 'Description is required';
  ...
  const handleSubmit = (e) => {
    ...
    if (validateForm()) {
      const filteredAmenities = room.amenities.filter(amenity => amenity.trim() !== '');
      ...
      addRoomToProperty(propertyId, newRoom);
      ...
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mb-2">
            You can upload up to 5 images. Click on an image to set it as the main image.
          </p>
          
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {room.images.map((_, index) => (
              <div key={index} className={`relative border-2 rounded-md ${room.mainImageIndex === index ? 'border-blue-500' : 'border-gray-300'}`}>
                <ImageUpload
                ...
                {room.images[index] && (
                  <div className="mt-2 flex justify-center">
                    <button
                      ...
                      className={`text-sm ${room.mainImageIndex === index ? 'bg-blue-500 text-white' : 'bg-gray-200'} py-1 px-2 rounded`}
                    >
                      {room.mainImageIndex === index ? 'Main Image' : 'Set as Main'}
                    </button>
                  </div>
                )}
              </div>
